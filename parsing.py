...
def import_data():
    with open('mapping_testoffres.json') as f:
        mapping = json.load(f)

    es = Elasticsearch([{'host': 'localhost', 'port': 9200}])
    es.indices.create(index='test_offres', ignore=400, body=mapping)
    
    max_offres = 100
    id_annonce = 1

    response = requests.get("http://api.dila.fr/opendata/api-boamp/annonces/search?criterion=securite")
    curseur_limit = response.json()["nbItemsExistants"]
    curseur = response.json()["nbItemsExistants"] - max_offres + 1
        ...
        while curseur < curseur_limit:
            ...
            for offre in response.json()["item"]:
                ...
                if annonce.status_code == 200:
                    es.index(index='test_offres', doc_type='annonce', id=id_annonce, body=annonce.json())
                    id_annonce += 1
                else:
                    #debug
